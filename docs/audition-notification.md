...
A Netlify scheduled function runs once per day at a fixed UTC time: 02:00 UTC
(put this in a Netlify parameter so it is easily modifiable). It
...
to add the subscriber. Buttondown sends a double opt-in confirmation email automatically.
Unsubscribe links in every email are handled by Buttondown — no custom unsubscribe
logic required.
...
resets filesystem timestamps at deploy time. The scheduled function reads audition data
from the GitHub Contents API (consistent with the existing `data.mjs` pattern), where
file-level timestamps reflect commit time — also unreliable for record-level change
detection. The `createdAt` and `updatedAt` fields in the JSON are the only reliable
source of record-level change information.
...
| ----------------------------------------- | ------------------- | --------------------------------------------------------------- |
| New subscriber                            | 201 Created         | "Thanks — check your inbox to confirm your subscription."       |
| Already subscribed (confirmed or pending) | 422                 | "That address is already subscribed."                           |
...
- **Buttondown broadcast API** — confirm the correct endpoint for sending a one-off
  broadcast to all subscribers (distinct from the subscriber management API) and whether
  the free tier imposes any rate or send limits relevant to daily digest frequency.
