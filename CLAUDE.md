...
Astro 6 static site covering Santa Cruz County theater — performances calendar, company directory, audition and event listings, and a services directory. Deployed to Netlify. TypeScript strict mode. No UI framework.

The site is structured as a hub (`/`) linking to four sections: **Calendar** (`/calendar`), **Companies** (`/companies`), **Events** (`/events`), and **Services** (`/services`). A fixed top nav (`SiteNav.astro`) is shared across all inner pages. An **About** page (`/about`) is linked from the home page footer and contains a Netlify contact form.
...
- `src/pages/` — file-based routes
  ...
  - `services.astro` — services directory at `/services`; three modes: index (category grid), filtered (`?category=`), deep-link (`?id=`)
...
- `netlify/functions/` — serverless functions. **New functions should use the Netlify Functions v2 interface** (`export default async (req: Request) => ...` + an `export const config: Config = {...}` for schedule/background, as in `subscribe.mts` and `sync-calendar-trigger.mts`/`sync-calendar-background.mts`) rather than the legacy `export const handler = schedule(...)` wrapper style (`send-digest.mts`, `data.mjs`) — for consistency going forward. Existing legacy-style functions are not being migrated proactively.
  ...
  - `sync-calendar-trigger.mts` / `sync-calendar-background.mts` — Google Calendar sync; see below
  - `lib/email-template.mts` — inline-styled email HTML templates for the digest and welcome email
  - `lib/activities-data.mts` — shared local activity data loading (fs-based, not GitHub API) for the digest and welcome email, and for the calendar sync
  - `lib/shows-data.mts` — fs-based shows/performances loader for the calendar sync, mirrors `getPerformances()` from `src/lib/data.ts`
  - `lib/venues-data.mts` — fs-based venue lookup for the calendar sync
  - `lib/company-data.mts` — fs-based company name/primary-venue lookup, shared by `activities-data.mts` and `shows-data.mts`
  - `lib/google-calendar.mts` — Google Calendar API client (auth, list/insert/update/delete, content hash)
...
- `public/admin/api.js` — shared `apiFetch` / `apiPut` / `apiUploadImage` / `resolveUserAccess` / `buildCompanyOptions`
...
- `public/admin/services.js` — services editor ES module (`mount` / `unmount`); admin-only dataset
...
`src/lib/audition-format.ts` — pure functions with no Astro or DOM dependencies. Exported: `fmt12`, `fmtTimeRange`, `fmtFullDate`, `fmtShortDate`, `fmtAudDateRange`, `rolesSum`, `renderMd`. `renderMd()` HTML-escapes then converts `**bold**`, `*italic*`, and `[link text](url)` — used for the audition `notes` field on the public page and in the editor's Preview modal. Used by:

- `src/pages/events.astro` (imported directly at build time)
- `netlify/functions/send-digest.mts` (resolved via esbuild bundling)
- `netlify/functions/sync-calendar-background.mts` (resolved via esbuild bundling; wraps `renderMd()` in a local `mdToHtml()` that also converts literal newlines to `<br>`, since Calendar's HTML description otherwise collapses multi-line notes)
...
Calendars ("Santa Cruz Theater — Shows"; "Santa Cruz Theater — Auditions & Events"),
via a service account. Fully implemented and live-tested against the real calendars;
not yet merged to `main`. Full design, field mapping, API gotchas, and test results:
...
run, or after any shared-field change) needs longer than a scheduled function's 30s
limit. The background function does a full idempotent reconciliation (insert/update/
delete) against a 30-day-past..18-month-future window every run, keyed by a
deterministic SHA-1 event id and a stored content hash — no cross-invocation state,
Google Calendar itself is the source of truth. `CALENDAR_SYNC_DRY_RUN=true` logs
planned changes without writing; `CALENDAR_SYNC_HEALTHCHECK_URL` gets a ping with a
per-calendar count summary (and failure details, id+error, on `{url}/fail`) every run.

## Services

### Data files

- `data/sc-theater-service-categories.json` — category hierarchy; hand-editable. Drives the index grid and filter dropdown.
- `data/sc-theater-services.json` — single flat file of all listings; edited via `/admin`. Top-level shape: `{ "listings": [ <ServiceListing>, ... ] }`.

`getServiceCategories()` and `getServices()` in `src/lib/data.ts` import these at build time via `import.meta.glob` (categories) and a direct glob (services).
...
`{ name?, tel?, email?, address? }` — at least `tel` or `email` required.

### Public page (`services.astro`) — three modes

**Index mode** (no params) — two-column category grid. Each top-level category links to `/services?category=<parent-id>` (matches all subcategory listings); each subcategory links to `/services?category=<sub-id>` (exact match). Categories with no active listings are rendered as non-interactive labels. The subtitle ("Production resources for…") is inside `#cat-index` and auto-hides with the index.

**Filtered mode** (`?category=<id>`) — category grid hidden; listing cards shown. A two-level checkbox dropdown (parent categories with collapsible subcategory rows; parent checkbox uses indeterminate state for partial selection) filters the list client-side. Filter changes close all open cards before re-filtering (matching Activities behaviour). When all subcategories are deselected the empty state shows with a "Return to Services index" link (`href="/services"`). The Services nav link is made clickable to return to index mode.

**Deep-link mode** (`?id=<timestamp>`) — single listing card shown expanded; all others hidden; filter bar and index hidden; page title set to the listing name at `18px` (matching Activities deep-link behaviour). Unrecognised id shows an error in `#svc-not-found`. URL format: `https://santacruz.theater/services?id=1735000000001`.

### Card display

Cards are collapsed by default. Filter changes reset all open cards to closed.

**Collapsed** — name + category pills inline (one pill per subcategory; text is `sub.description ?? sub.label`) + optional photo + contact info (name, tel, email, address).

**Expanded** — adds description (rendered markdown: `**bold**`, `*italic*`, `[text](url)`) and an optional "More info →" link.

#### `renderDesc()` — description markdown renderer

Defined inline in `services.astro` (server-side for Astro templates) and duplicated inside `services.js` (client-side for the Preview modal). Both implementations must be kept in sync. Transform order: HTML-escape → `**bold**` → `*italic*` → `[text](url)`. Regex for links: `\[([^\]]+)\]\((https?:\/\/[^)]+)\)` → `<a href="$2" target="_blank" rel="noopener">$1</a>`.

### Services editor (`services.js`)

Admin-only: only users with site-wide admin access see the Services tile in the hub.

- **Sidebar** — lists all listings (name + active/inactive); resizable via drag handle; **+ New** button creates a new record (hidden until data loads).
- **Categories widget** — two-level checkbox grid mirroring the category hierarchy. Parent checkbox selects/deselects all subcategories; shows indeterminate state when subs are partially checked. Stored value is always leaf-level subcategory ids only.
- **Photo** — file picker (`apiUploadImage` from `api.js`) uploads to `public/images/services/<filename>` on Save. Filename is editable before saving. Pending upload is shown as a preview thumbnail with a "Will be uploaded on Save" note. Dirty-check counts a pending upload as a change.
- **Description** — plain textarea; hint text documents `**bold**`, `*italic*`, and `[link text](url)` support.
- **Preview** — renders a modal via `openPreview()` / `closePreview()` showing the collapsed + expanded card as it will appear on `/services`. Uses `subPillMap` (built from loaded categories: `sub.description ?? sub.label`) and `renderDesc()` for description. Photo preview uses the pending upload's data URL if staged. Exposed on `window.svcEd`.
- **Active toggle** — in the record header; controls the `active` boolean.
- **Restore** — same pattern as Activities editor; appears when active record differs from its snapshot.
- **Save / dirty-check** — same pattern as Activities editor; `computeIsDirty()` compares snapshots (excluding `updatedAt`); `updatedAt` only advances for records that actually changed.
